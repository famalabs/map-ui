...
export const CustomFormattingToolbar = ({
  ...
  const defaultAvailableButtons: AvailableToolbarButtons = React.useMemo(
    () => ({
  ...
  const isEditorMounted = React.useMemo(() => {
    try {
      return Boolean(editor.prosemirrorView?.dom);
    } catch {
      return false;
    }
  }, [editor]);
  ...
  const toolbarButtons = React.useMemo(
    () => [
      <BlockTypeSelect
        ...
        items={
          defaultAvailableButtons.heading === 'full'
            ...
            : completeSelectOptions.slice(0, 4)
        }
      />,
      ...
      ...(isEditorMounted ? [<CreateLinkButton key={'link_button'} />] : []),
    ],
    [completeSelectOptions, defaultAvailableButtons.heading, isEditorMounted],
  );
  ...
  const mergedButtons = React.useMemo(() => {
