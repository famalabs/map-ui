...
export function SidebarFooter(props: SidebarFooterProps) {
  const {
    footerData: { itemsList, avatar, listProps = {}, listStyle = {} },
    sidebarOpen,
    setSidebarOpen,
    onSelectItem,
    onHoverItem,
  ...
  const [anchorElMenu, setAnchorElMenu] = React.useState<(EventTarget & HTMLDivElement) | null>(
    null,
  );
  ...
  return (
    <>
      <List>
        ...
        >
          <ListItemAvatar>
            <Avatar alt={avatar.username || ''} src={avatar.imageSrc || ''} />
          </ListItemAvatar>
          <ListItemText
            primary={
              <Typography
      ...
      {itemsList.length > 0 && (
        <Popover
          ...
          elevation={6}
        >
          <Box sx={{ width: mobile ? '100dvw' : 260 }}>
