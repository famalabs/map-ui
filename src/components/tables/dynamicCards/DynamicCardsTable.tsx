...
export function DynamicCardsTable<T extends Record<string, any>>(props: DynamicCardsProps<T>) {
  const {
    ...
    fetchInfo: { fetchData, isFetching },
    cardInfo,
  ...
  const { customPageItemCount = 6, customSelectPages = [6, 12, 24] } = standardOptions;
  ...
  const [selectedView, setSelectedView] = useState<InfiniteViewType>(
    viewType === 'dual' ? 'cards' : viewType,
  );
  ...
  const isTableFetching =
    tableVariant === 'standard'
      ? !isNextPageFetched && highestFetchedPage !== currentPage
      : isFetching;
  ...
  const fetchEvent = useCallback(
    async (fetchType: 'first' | 'next' = 'next') => {
      ...
      const itemsPerPage =
        tableVariant === 'standard' || loadingType === 'infiniteScroll'
          ? rowsPerPage * 2
          : rowsPerPage;
