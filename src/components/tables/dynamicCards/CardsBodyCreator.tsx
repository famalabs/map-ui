...
export function DefaultSkeletonCard() {
  return (
    <Card sx={{ minWidth: 150, minHeight: 300 }}>
      <CardMedia component={Skeleton} variant="rectangular" height={210} />
      <CardContent>
...
export function CardBodyCreator<T extends Record<string, any>>(props: CardBodyProps<T>) {
  ...
  const gridTemplateSizings = React.useMemo(
    () => ({
  ...
  const StandardBodyLayout = React.useMemo(
    () => (
      <>
        {currentPageRows.map((row) => (
          <CardItem key={row.id} {...row} />
        ))}
  ...
  return (
    <Box
      ...
      sx={{
        display: 'grid',
        gridTemplateColumns: viewType === 'cards' ? gridTemplateSizings : '1fr',
        gap: '32px 32px',
      }}
    >
